...
const businesses = [
  ...
  export const newYork=businesses.filter(businessObj=>{
      if(businessObj.addressStateCode=="NY"){
          return true
      }
      return false;
  })
  export const manufacturing=businesses.filter(businessObj=>{
      if (businessObj.companyIndustry=="Manufacturing"){return true}
  })
  export const agentInfo = businesses.map(businessObj=>{
    const agentNameString = businessObj.purchasingAgent.nameFirst+" "+ businessObj.purchasingAgent.nameLast
    const info={
      agentName:agentNameString,
      company:businessObj.companyName,
      phone:businessObj.phoneWork
    }
    return info
    
  })
  export const findItem=(item)=>{
    return businesses.find(business=>business.purchasingAgent.nameFirst==item || business.purchasingAgent.nameLast==item);
  }
  //business=> 